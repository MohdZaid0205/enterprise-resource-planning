...
public class GradebookView extends JPanel {
    ...
    public GradebookView(Instructor instructor) {
        ...
        JLabel title = new JLabel("Instructor Gradebook");
    ...
    private void loadSections() {
        ...
        if (mySections.isEmpty()) {
            JLabel empty = new JLabel("No sections assigned for grading.");
            ...
            for (String secId : mySections) {
                try {
                    ...
                    GradebookPanel panel = new GradebookPanel(section);
    ...
    private class GradebookPanel extends JPanel {
        ...
        private void toggleExpansion() {
            ...
            if (isExpanded) {
                toggleBtn.setText("Close Gradebook");
                loadContent();
            } else {
                toggleBtn.setText("Open Gradebook");
        ...
        private void loadContent() {
            ...
            weightsGrid.add(createStatLabel("Mid", section.getMidExams() + "%"));
            weightsGrid.add(createStatLabel("End", section.getEndExams() + "%"));
            weightsGrid.add(createStatLabel("Assign", section.getAssignments() + "%"));
            weightsGrid.add(createStatLabel("Proj", section.getProjects() + "%"));
            weightsGrid.add(createStatLabel("Bonus", section.getBonus() + "%"));
            ...
            slabsGrid.add(createStatLabel("A-", ">= " + section.getA_()));
            slabsGrid.add(createStatLabel("B", ">= " + section.getB()));
            slabsGrid.add(createStatLabel("B-", ">= " + section.getB_()));
            slabsGrid.add(createStatLabel("C", ">= " + section.getC()));
            slabsGrid.add(createStatLabel("C-", ">= " + section.getC_()));
            slabsGrid.add(createStatLabel("D", ">= " + section.getD()));
            slabsBox.add(slabsGrid);

            policyWrapper.add(weightsBox);
            policyWrapper.add(slabsBox);

            contentPanel.add(policyWrapper);
        ...
        private JPanel createCleanInfoBox(String title) {
            ...
            head.setForeground(StyleConstants.PRIMARY_COLOR);
        ...
        private JLabel createStatLabel(String title, String val) {
            return new JLabel("<html><div style='text-align:center; color:#7f8c8d; font-weight:bold; font-size:11px'>" + title +
                    "</div><div style='text-align:center; color:#2c3e50; font-size:12px'>" + val + "</div></html>", SwingConstants.CENTER);
        }
...
//                    section.setO(o);
    ...
    class ButtonEditor extends DefaultCellEditor {
        ...
        private GradebookPanel parentPanel;
        private List<StudentGradeRow> rows;
        private JTable table;

        public ButtonEditor(JCheckBox checkBox, List<StudentGradeRow> rows, String sectionId, GradebookPanel parentPanel) {
            super(checkBox);
            this.rows = rows;
            ...
            button.addActionListener(e -> {
                ...
                SwingUtilities.invokeLater(() -> {
                    if (currentRowData != null) {
                        parentPanel.showEditDialog(currentRowData, sectionId, parentPanel.section);
                    }
                });
            });
        }
